...
def upgrade() -> None:
    ...
    # 2. Chats Table
    chat_type_enum = sa.Enum("direct", "group", name="chat_type_enum")
    ...
    op.create_index("idx_chat_members_chat_user", "chat_members", ["chat_id", "user_id"])

    # 5. Messages Table
    message_type_enum = sa.Enum("text", "image", "video", "file", "voice", name="message_type_enum")
    ...
    # 6. Calls Table
    call_type_enum = sa.Enum("audio", "video", name="call_type_enum")
    call_type_enum.create(op.get_bind(), checkfirst=True)
    call_status_enum = sa.Enum(
        "initiated", "ringing", "accepted", "rejected", "ended", "missed", "busy", name="call_status_enum"
    )
    ...
    op.create_table(
        ...
        sa.Column("call_type", call_type_enum, nullable=False, server_default="audio"),
        sa.Column("status", call_status_enum, nullable=False, server_default="initiated"),
    ...
    op.create_index("idx_calls_caller_receiver", "calls", ["caller_id", "receiver_id"])
    op.create_index("idx_calls_started_at", "calls", ["started_at"])

    # 7. Device Tokens Table
    device_platform_enum = sa.Enum("android", "ios", "web", name="device_platform_enum")
    ...
    op.create_table(
        ...
        sa.Column("platform", device_platform_enum, nullable=False, server_default="android"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
